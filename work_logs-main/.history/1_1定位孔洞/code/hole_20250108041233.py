...
# 读取图像
image = cv2.imread('../img/image.jpg', cv2.IMREAD_GRAYSCALE)
...
circles = cv2.HoughCircles(
    ...
    param1=80,                     # Canny边缘检测的高阈值，低阈值自动设为高阈值的一半
    param2=40,                     # 累加器阈值，值越小检测到的圆越多（可能包含误检），值越大检测到的圆越少但更准确
    minRadius=int(radius_in_pixels - 20),  # 检测圆的最小半径（像素）
    maxRadius=int(radius_in_pixels + 40)   # 检测圆的最大半径（像素）
)
